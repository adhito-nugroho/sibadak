...
class DpnController
{
    ...
    private function model(): Dpn { return new Dpn($this->pdo()); }
    ...
    public function index(): void
    {
        ...
        $filters = [];
        if ($q !== '') $filters['q'] = $q;
        if (in_array($tahun, Dpn::availableYears(), true)) $filters['tahun'] = $tahun;
        ...
        $filterTahun = $tahun;
        $filterQ = $q;
        $yearOptions = Dpn::availableYears();

        ob_start();
        require view_path('dpn/index.php');
    ...
    public function create(): void
    {
        ...
        $pageTitle = 'Tambah DPN';
        $activeNav = 'dpn';
        $dpn = null;
        $yearOptions = Dpn::availableYears();

        ob_start();
        require view_path('dpn/form.php');
        $content = ob_get_clean();
        require view_path('layouts/main.php');
    }
    ...
    public function store(): void
    {
        ...
        if ($lokasi === '') { set_flash('error', 'Lokasi wajib diisi.'); header('Location: ' . APP_URL . '/dpn/create'); exit; }
        $tahun = (int) req_str('tahun');
        if (!in_array($tahun, Dpn::availableYears(), true)) { set_flash('error', 'Tahun tidak valid.'); header('Location: ' . APP_URL . '/dpn/create'); exit; }
        ...
        log_activity($pdo, 'dpn', 'create', 'Tambah DPN ID ' . $id);
        set_flash('success', 'Data DPN berhasil ditambahkan.');
        header('Location: ' . APP_URL . '/dpn/' . $id);
        exit;
    }
    ...
    public function show(int $id): void
    {
        ...
        $pageTitle = 'Detail DPN';
        $activeNav = 'dpn';
        ob_start();
        require view_path('dpn/show.php');
        $content = ob_get_clean();
        require view_path('layouts/main.php');
    }
    ...
    public function update(int $id): void
    {
        ...
        $lokasi = req_str('lokasi');
        if ($lokasi === '') { set_flash('error', 'Lokasi wajib diisi.'); header('Location: ' . APP_URL . '/dpn/' . $id . '/edit'); exit; }
        $tahun = (int) req_str('tahun');
        ...
        log_activity($pdo, 'dpn', 'update', 'Update DPN ID ' . $id);
        set_flash('success', 'Data DPN berhasil diperbarui.');
        header('Location: ' . APP_URL . '/dpn/' . $id);
        exit;
    }
    ...
    public function delete(int $id): void
    {
        ...
        log_activity($pdo, 'dpn', 'delete', 'Hapus DPN ID ' . $id);
        set_flash('success', 'Data DPN berhasil dihapus.');
