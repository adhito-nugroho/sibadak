<?php
/** @var list<array{id:int,nama:string,urutan:int,is_active:int}> $komoditas */
/** @var int $tahun */
...
<div class="max-w-3xl mx-auto space-y-5 fade-up">
    <div class="flex items-center justify-between">
        <h2 class="text-sm font-semibold text-gray-800">Komoditas & Target DPA — HHK</h2>
    ...
    <div class="bg-white rounded-xl border border-gray-100 p-4">
        ...
        <form method="post" action="<?= htmlspecialchars(APP_URL . '/laporan/komoditas-hhk/save', ENT_QUOTES, 'UTF-8') ?>" class="flex flex-wrap items-end gap-3">
            ...
            <div class="flex-1 min-w-[180px]">
                ...
                <input type="text" name="nama" required placeholder="Jati, Sengon, Bambu..." class="w-full text-xs px-3 py-2 rounded-lg border border-gray-200 outline-none">
            </div>
            <div class="w-24">
    ...
    <div class="bg-white rounded-xl border border-gray-100 overflow-hidden">
        <div class="px-4 py-3 border-b border-gray-100 bg-gray-50/50">
            <h3 class="text-xs font-semibold text-gray-800">Daftar Jenis Kayu (Kolom Laporan)</h3>
            <p class="text-[10px] text-gray-400 mt-0.5">Urutan menentukan posisi kolom di laporan. Toggle untuk aktif/nonaktif.</p>
        </div>
        <table class="data-table w-full">
            <thead><tr class="border-b border-gray-100"><th class="px-4 py-2.5 text-left">Urutan</th><th class="px-4 py-2.5 text-left">Nama</th><th class="px-4 py-2.5 text-center">Status</th><th class="px-4 py-2.5 text-right">Aksi</th></tr></thead>
            ...
            <tr style="<?= !$aktif ? 'opacity:0.4' : '' ?>">
                ...
                <td class="px-4 py-2 text-right">
                    <form method="post" action="<?= htmlspecialchars(APP_URL . '/laporan/komoditas-hhk/' . (int)$k['id'] . '/toggle', ENT_QUOTES, 'UTF-8') ?>" class="inline">
                        <input type="hidden" name="csrf_token" value="<?= htmlspecialchars(csrf_token(), ENT_QUOTES, 'UTF-8') ?>">
    ...
    <div class="bg-white rounded-xl border border-gray-100 p-4">
        ...
        <form method="post" action="<?= htmlspecialchars(APP_URL . '/laporan/target-hhk/save', ENT_QUOTES, 'UTF-8') ?>" class="space-y-3">
            <input type="hidden" name="csrf_token" value="<?= htmlspecialchars(csrf_token(), ENT_QUOTES, 'UTF-8') ?>">
            <div class="flex items-end gap-3 mb-4">
                ...
                <p class="text-[10px] text-gray-400">Isi target DPA dalam m³ per jenis kayu untuk tahun yang dipilih.</p>
            </div>
            <div class="grid grid-cols-2 md:grid-cols-3 gap-3">
                <?php foreach ($komoditas as $k): if (!(int)($k['is_active'] ?? 1)) continue; ?>
                <div>
                    <label class="block text-[10px] font-medium text-gray-600 mb-1"><?= htmlspecialchars($k['nama'], ENT_QUOTES, 'UTF-8') ?> (m³)</label>
