<?php
/** @var array|null $gully */
$isEdit = $gully !== null;
$action = $isEdit ? APP_URL . '/gully-plug/' . (int) $gully['id'] . '/update' : APP_URL . '/gully-plug/store';
$fv = static fn(string $f) => htmlspecialchars((string) ($gully[$f] ?? ''), ENT_QUOTES, 'UTF-8');
?>
<div class="max-w-2xl mx-auto fade-up">
    <div class="flex items-center justify-between mb-5">
        <h2 class="text-sm font-semibold text-gray-800"><?= $isEdit ? 'Edit Gully Plug' : 'Tambah Gully Plug' ?></h2>
        <a href="<?= htmlspecialchars($isEdit ? APP_URL . '/gully-plug/' . (int) $gully['id'] : APP_URL . '/gully-plug', ENT_QUOTES, 'UTF-8') ?>" class="text-xs text-forest-600 font-medium hover:underline">← Kembali</a>
    </div>
    <form method="post" action="<?= htmlspecialchars($action, ENT_QUOTES, 'UTF-8') ?>" class="bg-white rounded-xl border border-gray-100 p-4 space-y-4">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
                ...
                <select name="tahun" required class="w-full text-xs px-3 py-2 rounded-lg border border-gray-200 outline-none">
                    <?php foreach ($yearOptions as $th): ?><option value="<?= $th ?>" <?= $isEdit && (int) $gully['tahun'] === $th ? 'selected' : '' ?>><?= $th ?></option><?php endforeach; ?>
                </select>
            </div>
            <div>
                <label class="block text-[10px] font-medium text-gray-500 uppercase tracking-wide mb-1">Jumlah Unit</label>
                <input type="number" name="jumlah_unit" value="<?= $isEdit ? (int) $gully['jumlah_unit'] : 1 ?>" min="1" class="w-full text-xs px-3 py-2 rounded-lg border border-gray-200 outline-none">
            </div>
            <div>
        ...
        <div class="flex justify-end pt-2">
            <button type="submit" class="px-4 py-2 rounded-lg bg-forest-800 text-white text-xs font-medium hover:bg-forest-700"><?= $isEdit ? 'Simpan Perubahan' : 'Simpan Gully Plug' ?></button>
        </div>
    </form>
</div>
