...
<div class="flex flex-wrap items-center justify-between gap-4 mb-5 fade-up">
    <div>
        <h2 class="text-sm font-semibold text-gray-800">Data Gully Plug</h2>
        <p class="text-[10px] text-gray-400 mt-0.5"><?= format_id($total) ?> entri</p>
    </div>
    <?php if ($canMut): ?>
    <a href="<?= htmlspecialchars(APP_URL . '/gully-plug/create', ENT_QUOTES, 'UTF-8') ?>" class="flex items-center gap-1.5 bg-forest-800 hover:bg-forest-700 text-white text-xs font-medium px-3.5 py-2 rounded-lg transition-colors"><i class="ti ti-plus text-sm leading-none"></i> Tambah Gully Plug</a>
    <?php endif; ?>
</div>

<form method="get" action="<?= htmlspecialchars(APP_URL . '/gully-plug', ENT_QUOTES, 'UTF-8') ?>" class="bg-white rounded-xl border border-gray-100 p-4 mb-4 fade-up delay-1 flex flex-wrap items-end gap-3">
    <div class="w-32">
        ...
        <table class="data-table w-full">
            ...
            <?php else: foreach ($rows as $r): ?>
                <tr class="hover:bg-forest-50/50 transition-colors">
                    ...
                    <td class="px-4 py-2.5 text-right"><a href="<?= htmlspecialchars(APP_URL . '/gully-plug/' . (int) $r['id'], ENT_QUOTES, 'UTF-8') ?>" class="text-[10px] font-medium text-forest-600 hover:underline">Detail</a></td>
                </tr>
