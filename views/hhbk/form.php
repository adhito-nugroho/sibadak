...
/** @var array|null $hhbk */
$isEdit = $hhbk !== null;
$action = $isEdit ? APP_URL . '/hhbk/' . (int) $hhbk['id'] . '/update' : APP_URL . '/hhbk/store';
$fv = static fn(string $f) => htmlspecialchars((string) ($hhbk[$f] ?? ''), ENT_QUOTES, 'UTF-8');
$opKab = user_role() === 'operator' ? user_kabupaten_id() : null;
?>
<div class="max-w-2xl mx-auto fade-up">
    <div class="flex items-center justify-between mb-5">
        <h2 class="text-sm font-semibold text-gray-800"><?= $isEdit ? 'Edit HHBK' : 'Tambah HHBK' ?></h2>
        <a href="<?= htmlspecialchars($isEdit ? APP_URL . '/hhbk/' . (int) $hhbk['id'] : APP_URL . '/hhbk', ENT_QUOTES, 'UTF-8') ?>" class="text-xs text-forest-600 font-medium hover:underline">← Kembali</a>
    </div>
    ...
    <form method="post" action="<?= htmlspecialchars($action, ENT_QUOTES, 'UTF-8') ?>" class="bg-white rounded-xl border border-gray-100 p-4 space-y-5">
        ...
        <div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-3">
                <div>
                    ...
                    <select name="kabupaten_id" id="hhbk_kab" required <?= $opKab !== null ? 'disabled' : '' ?> class="w-full text-xs px-3 py-2 rounded-lg border border-gray-200 outline-none disabled:bg-gray-50">
                        <option value="">— Pilih —</option>
                        <?php foreach ($kabupatenList as $kb): ?>
                        <option value="<?= (int) $kb['id'] ?>" <?= ($isEdit && (int) ($hhbk['kabupaten_id'] ?? 0) === (int) $kb['id']) || ($opKab !== null && (int) $kb['id'] === $opKab) ? 'selected' : '' ?>><?= htmlspecialchars($kb['nama'], ENT_QUOTES, 'UTF-8') ?></option>
                        <?php endforeach; ?>
                    </select>
                ...
                <div>
                    ...
                    <select name="kecamatan_id" id="hhbk_kec" class="w-full text-xs px-3 py-2 rounded-lg border border-gray-200 outline-none">
                        <option value="">— Pilih kabupaten dulu —</option>
                    </select>
                </div>
                <div>
                    <label class="block text-[10px] font-medium text-gray-500 uppercase tracking-wide mb-1">Desa</label>
                    <select name="desa_id" id="hhbk_desa" class="w-full text-xs px-3 py-2 rounded-lg border border-gray-200 outline-none">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
                <div>
                    ...
                    <div style="position:relative;">
                        <input type="text" name="nama_kth" id="hhbk_nama_kth" value="<?= $fv('nama_kth') ?>" autocomplete="off"
                        ...
                        <div id="hhbk_kth_dropdown" class="hidden absolute z-50 mt-1 w-full max-h-52 overflow-y-auto bg-white border border-gray-200 rounded-lg shadow-lg"></div>
                    </div>
                </div>
                <div>
                    ...
                    <select name="penyuluh_id" required class="w-full text-xs px-3 py-2 rounded-lg border border-gray-200 outline-none bg-white">
                        ...
                        <option value="<?= (int) $p['id'] ?>" <?= ($isEdit && (int) ($hhbk['penyuluh_id'] ?? 0) === (int) $p['id']) ? 'selected' : '' ?>>
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-3">
                <div>
                    ...
                    <select name="bulan" required class="w-full text-xs px-3 py-2 rounded-lg border border-gray-200 outline-none">
                        <?php foreach ($bulanOptions as $b => $n): ?>
                        <option value="<?= $b ?>" <?= $isEdit && (int) $hhbk['bulan'] === $b ? 'selected' : '' ?>><?= $n ?></option>
                        <?php endforeach; ?>
                    </select>
                </div>
                <div>
                    ...
                    <select name="tahun" required class="w-full text-xs px-3 py-2 rounded-lg border border-gray-200 outline-none">
                        ...
                        <option value="<?= $y ?>" <?= $isEdit && (int) $hhbk['tahun'] === $y ? 'selected' : '' ?>><?= $y ?></option>
                        <?php endforeach; ?>
                    </select>
                </div>
        ...
        <div class="flex justify-end">
            <button type="submit" class="px-4 py-2 rounded-lg bg-forest-800 text-white text-xs font-medium hover:bg-forest-700">
                <?= $isEdit ? 'Simpan Perubahan' : 'Simpan & Lanjut Isi Komoditas' ?>
            </button>
        </div>
    </form>
</div>
...
(function(){
    ...
    const kabSel = document.getElementById('hhbk_kab');
    const kecSel = document.getElementById('hhbk_kec');
    const desSel = document.getElementById('hhbk_desa');
    const kthInput = document.getElementById('hhbk_nama_kth');
    const kthDropdown = document.getElementById('hhbk_kth_dropdown');
    let pelaksanaList = [];
    let debounceTimer = null;

    async function loadKec(kabId) {
        kecSel.innerHTML = '<option value="">Memuat...</option>';
        desSel.innerHTML = '<option value="">—</option>';
        ...
        data.forEach(r => {
            const o = new Option(r.nama, r.id);
            <?= $isEdit ? 'if(parseInt(r.id)===' . (int)($hhbk['kecamatan_id'] ?? 0) . ')o.selected=true;' : '' ?>
            kecSel.appendChild(o);
        });
        <?php if ($isEdit && !empty($hhbk['kecamatan_id'])): ?>kecSel.dispatchEvent(new Event('change'));<?php endif; ?>
    }
    ...
    async function loadDesa(kecId) {
        ...
        data.forEach(r => {
            const o = new Option(r.nama, r.id);
            <?= $isEdit ? 'if(parseInt(r.id)===' . (int)($hhbk['desa_id'] ?? 0) . ')o.selected=true;' : '' ?>
