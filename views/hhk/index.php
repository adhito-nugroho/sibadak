...
<div class="page-toolbar fade-up">
    <p class="page-toolbar-meta">
        Hasil Hutan Kayu · <strong><?= format_id($total) ?></strong> entri · halaman <?= $cur ?>/<?= max(1, $pages) ?>
    </p>
    <?php if ($canMut): ?>
    <a href="<?= htmlspecialchars(APP_URL . '/hhk/create', ENT_QUOTES, 'UTF-8') ?>" class="btn-primary-add">
        <i class="ti ti-plus text-sm leading-none"></i> Tambah HHK
    </a>
    <?php endif; ?>
</div>

<form method="get" action="<?= htmlspecialchars(APP_URL . '/hhk', ENT_QUOTES, 'UTF-8') ?>" class="filter-bar fade-up delay-1">
    ...
    <div class="filter-actions">
        ...
        <a href="<?= htmlspecialchars(APP_URL . '/hhk', ENT_QUOTES, 'UTF-8') ?>" class="btn-reset">Reset</a>
    </div>
</form>
...
<div class="stat-card overflow-hidden fade-up delay-2">
        ...
        <table class="data-table">
            <thead>
                <tr>
                    ...
                    <th class="text-right">Total Bulan Ini (m³)</th>
                    <th class="text-right">Total s.d. Bulan Ini (m³)</th>
            ...
            <tbody>
                ...
                <tr>
                    <td colspan="7" class="cell-empty">
                        <div class="empty-state">
                            <div class="empty-state-icon"><i class="ti ti-tree"></i></div>
                            <p class="empty-state-title">Belum ada HHK yang cocok</p>
                ...
                <tr>
                    ...
                    <td class="text-right"><?= format_id((float) $r['total_bulan_ini_m3'], 2) ?></td>
                    <td class="text-right"><?= format_id((float) $r['total_sd_bulan_ini_m3'], 2) ?></td>
                    <td class="col-aksi">
                        <div class="inline-flex items-center gap-1 justify-end">
                            <a href="<?= htmlspecialchars(APP_URL . '/hhk/' . (int) $r['id'], ENT_QUOTES, 'UTF-8') ?>" class="btn-icon" title="Detail"><i class="ti ti-eye"></i></a>
                            <?php if ($canMut): ?>
                            <a href="<?= htmlspecialchars(APP_URL . '/hhk/' . (int) $r['id'] . '/edit', ENT_QUOTES, 'UTF-8') ?>" class="btn-icon btn-icon-muted" title="Edit"><i class="ti ti-pencil"></i></a>
                            <?php endif; ?>
                        </div>
                    </td>
                </tr>
    ...
    <div class="pager">
        ...
        <div class="pager-links">
            <?php if ($cur > 1): ?><a href="<?= htmlspecialchars(APP_URL . '/hhk?' . $qs($cur - 1), ENT_QUOTES, 'UTF-8') ?>">« Prev</a><?php endif; ?>
            <?php if ($cur < $pages): ?><a href="<?= htmlspecialchars(APP_URL . '/hhk?' . $qs($cur + 1), ENT_QUOTES, 'UTF-8') ?>">Next »</a><?php endif; ?>
        </div>
    </div>
    <?php endif; ?>
</div>
