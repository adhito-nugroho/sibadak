...
<div class="flex flex-wrap items-center justify-between gap-4 mb-5 fade-up">
    <div>
        <h2 class="text-sm font-semibold text-gray-800">Hasil Hutan Kayu (HHK)</h2>
        <p class="text-[10px] text-gray-400 mt-0.5"><?= format_id($total) ?> entri</p>
    </div>
    <?php if ($canMut): ?>
    <a href="<?= htmlspecialchars(APP_URL . '/hhk/create', ENT_QUOTES, 'UTF-8') ?>" class="flex items-center gap-1.5 bg-forest-800 hover:bg-forest-700 text-white text-xs font-medium px-3.5 py-2 rounded-lg transition-colors"><i class="ti ti-plus text-sm"></i> Tambah HHK</a>
    <?php endif; ?>
</div>

<form method="get" action="<?= htmlspecialchars(APP_URL . '/hhk', ENT_QUOTES, 'UTF-8') ?>" class="bg-white rounded-xl border border-gray-100 p-4 mb-4 fade-up delay-1 flex flex-wrap items-end gap-3">
    <?php if (user_role() !== 'operator'): ?>
    <div class="w-36"><label class="block text-[10px] font-medium text-gray-500 uppercase tracking-wide mb-1">Kabupaten</label>
    ...
    <?php endif; ?>
    <div class="w-24"><label class="block text-[10px] font-medium text-gray-500 uppercase tracking-wide mb-1">Tahun</label>
    ...
    <div class="flex-1 min-w-[180px]"><label class="block text-[10px] font-medium text-gray-500 uppercase tracking-wide mb-1">Cari</label>
        <input type="text" name="q" value="<?= htmlspecialchars($filterQ, ENT_QUOTES, 'UTF-8') ?>" placeholder="Nama KTH / penyuluh" class="w-full text-xs px-3 py-2 rounded-lg border border-gray-200 outline-none"></div>
    ...
    <div class="overflow-x-auto">
        <table class="data-table w-full">
            <thead><tr class="border-b border-gray-100">
                ...
                <th class="px-4 py-2.5 text-left">Wilayah</th>
                <th class="px-4 py-2.5 text-left">Penyuluh</th>
                <th class="px-4 py-2.5 text-left">Periode</th>
                <th class="px-4 py-2.5 text-right">Total Bulan Ini (m³)</th>
                <th class="px-4 py-2.5 text-right">Total s.d. Bulan Ini (m³)</th>
            ...
            <?php else: foreach ($rows as $r): ?>
                <tr class="hover:bg-forest-50/50 transition-colors">
                    ...
                    <td class="px-4 py-2.5 text-xs text-right text-gray-700"><?= format_id((float) $r['total_bulan_ini_m3'], 2) ?></td>
                    <td class="px-4 py-2.5 text-xs text-right text-gray-700"><?= format_id((float) $r['total_sd_bulan_ini_m3'], 2) ?></td>
                    <td class="px-4 py-2.5 text-right"><a href="<?= htmlspecialchars(APP_URL . '/hhk/' . (int) $r['id'], ENT_QUOTES, 'UTF-8') ?>" class="text-[10px] font-medium text-forest-600 hover:underline">Detail</a></td>
                </tr>
