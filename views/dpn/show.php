...
<div class="max-w-2xl mx-auto space-y-5 fade-up">
    <div class="flex flex-wrap items-start justify-between gap-4">
        <div>
            <p class="text-[10px] text-gray-400 uppercase tracking-wide font-medium">Dam Penahan</p>
            <h2 class="text-sm font-semibold text-gray-800 mt-1"><?= htmlspecialchars((string) ($row['sasaran'] ?? 'DPN #' . $row['id']), ENT_QUOTES, 'UTF-8') ?></h2>
        </div>
        <div class="flex gap-2">
            <a href="<?= htmlspecialchars(APP_URL . '/dpn', ENT_QUOTES, 'UTF-8') ?>" class="px-3 py-1.5 text-xs rounded-lg border border-gray-200 text-gray-600 hover:bg-gray-50">← Daftar</a>
            <?php if ($canMut): ?>
            <a href="<?= htmlspecialchars(APP_URL . '/dpn/' . (int) $row['id'] . '/edit', ENT_QUOTES, 'UTF-8') ?>" class="px-3 py-1.5 text-xs rounded-lg bg-forest-800 text-white hover:bg-forest-700">Edit</a>
            <?php endif; ?>
        </div>
    </div>
    ...
    <div class="flex justify-end">
        <form method="post" action="<?= htmlspecialchars(APP_URL . '/dpn/' . (int) $row['id'] . '/delete', ENT_QUOTES, 'UTF-8') ?>" onsubmit="return confirm('Hapus data DPN ini?');">
            <input type="hidden" name="csrf_token" value="<?= htmlspecialchars(csrf_token(), ENT_QUOTES, 'UTF-8') ?>">
            <button type="submit" class="text-xs text-red-600 hover:text-red-700 font-medium px-3 py-1.5 rounded-lg border border-red-200 hover:bg-red-50">Hapus DPN</button>
        </form>
    </div>
    <?php endif; ?>
</div>
